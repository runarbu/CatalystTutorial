...
the moose cage (Mynd you, moose bites kan be pretty nasti!). For example: 
...
C<check_user_roles> has a sister method, C<assert_user_roles>, which throws an
...
The role list behaves in the same way as C<check_user_roles>. However, the ACL
