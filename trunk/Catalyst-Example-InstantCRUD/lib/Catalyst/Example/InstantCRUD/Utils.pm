...
" . $rels{$c};
        ...
        $config{$class}{fields}{$additional_col->{name}} = $additional_col if $additional_col;
        for my $fname (keys %{$config{$class}{fields}}){ 
            ...
            for my $not_for_config (qw/size data_type is_auto_increment default_value is_nullable/){
    ...
    for my $s ( @classes = $schema->sources ) {
        ...
        if (scalar(@relationships) == 2 && scalar(@cols) == 2 &&
            ...
            if ($other_rel_info1 && $other_rel_info2) {
                ...
                delete $config{_strip_class( $other_class1 )}{fields}{$other_rel_name};
                delete $config{_strip_class( $other_class2 )}{fields}{$other_rel_name};
    ...
    for my $class ( keys %config ){
        $ret{config}{$class}{fields} = [ values %{$config{$class}{fields}} ];
